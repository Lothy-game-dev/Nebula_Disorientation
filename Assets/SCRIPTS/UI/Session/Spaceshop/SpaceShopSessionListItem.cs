...
public class SpaceShopSessionListItem : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (GenerateTimer > 0f)
        {
            ...
            if (!AlreadyClear)
            {
                ...
                Loading.transform.GetChild(0).GetComponent<SpaceShopSessionLoading>().LoadTime = 0.75f;
            }
        }
        else
        {
    ...
    public void ChooseItem(GameObject go)
    {
        ...
        if (ItemAfterGen.Contains(go))
        {
            ...
            go.GetComponent<Image>().color = Color.green;
            Information.GetComponent<SpaceShopSessionInformation>().ShowInformationOfItem(go.name);
        }
        else
        {
